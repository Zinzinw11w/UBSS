        ...
        <div className="mt-2 mb-2">
          <div className="bg-gray-800 rounded-lg px-1 py-1 flex items-center justify-between">
            ...
            <div className="grid grid-cols-3 gap-1 mb-2">
              {(Array.isArray(marketData) ? marketData.slice(0, 3) : []).map((item) => (
                <Link
                  key={item.id}
                  to={`/trading/${activeTab.toLowerCase()}/${item.pair}`}
                  className="block"
                >
                  <div className="bg-gray-100 rounded-md p-1.5 border border-gray-200 hover:bg-gray-200 transition-colors cursor-pointer">
